(() => {
  ...
  async function loadBuildCommit() {
    try {
      const resp = await fetch("/app/commit.txt", { cache: "no-store" });
      if (!resp.ok) return;
      const text = (await resp.text()).trim();
      if (!text || text.startsWith("<")) return;
      state.buildCommit = text;
    } catch (_err) {
      // Optional debug signal; keep UI usable if commit file is unavailable.
    }
  }
