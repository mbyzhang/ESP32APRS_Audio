...
static bool isAutoGeneratedApName(const char *ssid, const char *baseName)
{
    if (ssid == nullptr || baseName == nullptr)
    {
        return false;
    }

    size_t baseLen = strlen(baseName);
    if (strncmp(ssid, baseName, baseLen) != 0)
    {
        return false;
    }
    ...
    if (ssid[baseLen] != '-' || strlen(ssid) != (baseLen + 5))
    {
        return false;
    }

    for (size_t i = baseLen + 1; i < baseLen + 5; i++)
    {
        if (!isHexDigitChar(ssid[i]))
        {
            return false;
        }
    }
...
static bool applyUniqueDefaultApSsid()
{
    static const char *baseApName = "ESP32APRS_Audio";
    if (!isAutoGeneratedApName(config.wifi_ap_ssid, baseApName))
    {
        return false;
    }

    uint64_t chipid = ESP.getEfuseMac();
    uint16_t lastMacWord = (uint16_t)(chipid & 0xFFFF);
    char targetSsid[sizeof(config.wifi_ap_ssid)];
    snprintf(targetSsid, sizeof(targetSsid), "%s-%04X", baseApName, lastMacWord);

    if (strncmp(config.wifi_ap_ssid, targetSsid, sizeof(config.wifi_ap_ssid)) == 0)
    {
        return false;
    }

    strlcpy(config.wifi_ap_ssid, targetSsid, sizeof(config.wifi_ap_ssid));
    return true;
}
...
// }
    ...
    bool saveConfigAfterBootAdjustments = false;
    ...
    {
        log_w("Applied KV4P HT 2.0D GPIO defaults");
        saveConfigAfterBootAdjustments = true;
    }
#endif

    if (applyUniqueDefaultApSsid())
    {
        log_i("Set AP SSID to unique name: %s", config.wifi_ap_ssid);
        saveConfigAfterBootAdjustments = true;
    }

    if (saveConfigAfterBootAdjustments)
    {
        saveConfiguration("/default.cfg", config);
    }
