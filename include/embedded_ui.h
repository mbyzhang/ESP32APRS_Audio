...
<div id="mapOverlay" hidden>
  ...
  <button id="mapPickBtn" class="ghost" type="button" title="Pick a location to send">📍 send from map</button>
  <div id="map"></div>
  <div id="mapStatus" class="dim">loading map…</div>
  <div id="mapPickBar" hidden>
    <span id="mapPickCoords" class="dim">tap the map to place a marker</span>
    <button id="mapPickSend" type="button" disabled>Send this location</button>
    <button id="mapPickCancel" class="ghost" type="button">cancel</button>
  </div>
</div>
...
.msg .head .status.ok{color:#2ea043}
.msg .head .status.warn{color:#f0883e}
.msg .head .status.err{color:#f85149}
/* Inbound bubble we've already ACK'd back to the sender — left border accent. */
.msg.acked{border-left:3px solid #2ea043}
...
#mapPickBtn{
  position:absolute;top:calc(8px + env(safe-area-inset-top));left:8px;z-index:12;
  background:#0d1117e0;border:1px solid #30363d;border-radius:999px;
  padding:6px 10px;font-size:12px
}
#mapPickBtn.on{background:#1f6feb;border-color:#1f6feb;color:#fff}
#mapPickBar{
  position:absolute;left:0;right:0;bottom:0;z-index:12;
  display:flex;gap:8px;align-items:center;justify-content:space-between;
  padding:8px 10px;background:#0d1117;border-top:1px solid #30363d
}
#mapPickBar[hidden]{display:none}
#mapPickBar #mapPickCoords{font-variant-numeric:tabular-nums;font-size:12px;flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
#mapPickBar #mapPickSend{background:#1f6feb;border-color:#1f6feb;color:#fff}
#mapPickBar #mapPickSend[disabled]{opacity:.5}
